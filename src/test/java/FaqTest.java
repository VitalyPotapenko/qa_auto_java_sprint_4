...
public class FaqTest {
    ...
    public static Object[][] getTestData() {
        return new Object[][]{{"Сколько это стоит? И как оплатить?", "Сутки — 400 рублей. Оплата курьеру — наличными или картой."},
    ...
    public void faqAnswerMustBeReturnedAsResponseToFaqQuestion() {
        ...
        String accordionButtonId = steps.open(mainPage.getUrl()).click(mainPage.getFaqAccordionButtonByQuestionText(faqQuestion)).getId(mainPage.getFaqAccordionButtonByQuestionText(faqQuestion));
        String actualAnswer = steps.getText(mainPage.getFaqAccordionPanel(accordionButtonId));
        assertEquals("faqAnswerMustBeReturnedAsResponseToFaqQuestion failed for run with " + driver.toString() + "expected answer: \"" + expectedAnswer + "\" but was \"" + actualAnswer + "\"", expectedAnswer, actualAnswer);
    }
